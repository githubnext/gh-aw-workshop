...
The default token inside a Codespace usually has enough access to work with your repository. However, it may not have the `actions:write` and `workflows:write` permissions that `gh aw run` needs.
