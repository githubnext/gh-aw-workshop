...
|------|------|------|------|
| **GitHub Codespaces** | Your cloud development environment when you choose the browser-based setup path. Pre-launched in golden-ticket workshops. You use it for a ready-to-go development environment in your browser. | Steps 2–14: writing, compiling, and running workflows | [GitHub Codespaces docs](https://docs.github.com/en/codespaces) |
| **Visual Studio Code (VS Code)** | The editor experience inside Codespaces (and optionally on your local machine). You use it to browse files, edit workflows, and keep a terminal open beside your work. | Editing workflow files and reading output | [Visual Studio Code docs](https://code.visualstudio.com/docs) |
| **Terminal (command line)** | The shell where you run workshop commands (`gh`, `gh aw`, `git`, and more). You use it whenever the workshop asks you to run `gh`, `gh aw`, or `git` commands. | Any step that shows a `bash` code block | [GitHub CLI manual](https://cli.github.com/manual/) |
| **GitHub CLI (`gh`)** | GitHub's official CLI, required for this workshop. Pre-installed in the Codespace. You use it for GitHub-specific terminal tasks like authentication checks, repository shortcuts, and workflow commands. | Starting at Step 6 (install the extension) | [GitHub CLI docs](https://cli.github.com/manual/) |
| **`gh-aw` CLI extension** | The GitHub Agentic Workflows extension you install and use in the terminal to compile workflow files. You use it to compile agentic workflow files. | Step 6 onward | [Install `gh-aw`](https://github.com/github/gh-aw#readme) |
| **GitHub Copilot CLI** | Copilot in the terminal for AI-assisted command and development help. The primary AI surface in this workshop. You use it when you want AI help inside the terminal. | Any step that shows a `prompt` code block | [GitHub Copilot CLI docs](https://docs.github.com/en/copilot/concepts/agents/copilot-cli/about-copilot-cli) |
| **GitHub Copilot app** | The GitHub Copilot desktop and web application where you can open repositories, start agent sessions, steer coding tasks, and manage pull requests. You use it when you want to steer repository sessions and review pull requests from a Copilot workspace. | Optional; side quests cover this surface | [GitHub Copilot app](https://github.com/features/ai/github-app) |
| **Claude** | Anthropic's AI model family available in some GitHub Copilot and agentic workflow contexts. May appear as a model option that reads a brief, reasons through a task, and produces an output. | Steps that use a non-default model | [Claude documentation](https://docs.anthropic.com/) |
| **OpenAI Codex** | OpenAI coding model family that can be used in coding and agent workflows. May appear as a coding-focused model option that reads files and suggests edits. | Steps that use a non-default model | [OpenAI Codex CLI repository](https://github.com/openai/codex#readme) |

### :dart: Quick self-check

Match each task to the tool you'd use. Answers are below the table.

| Task | Tool? |
|------|-------|
| 1. Compile an agentic workflow file | ? |
| 2. Open a browser-based dev environment with no local setup | ? |
| 3. Get AI help without leaving the terminal | ? |

<details>
<summary>Check your answers</summary>

1. `gh aw` — the CLI extension that compiles workflow files.
2. GitHub Codespaces — the cloud development environment.
3. GitHub Copilot CLI — AI-assisted help inside the terminal.
...
These visuals are simplified mental models, not literal product screenshots. Use them to recognize what each name refers to when it appears in later steps. Expand only the tools you need.
...
<details>
<summary>Workshop tools and model options (gh, gh-aw, Copilot CLI, Copilot app, Claude, OpenAI Codex)</summary>
...
- [ ] You completed the quick self-check and matched each task to its tool
