...
> - **Path C — UI only (Copilot Chat / CCA):** this step's `gh aw` commands require a terminal. If you stay UI-only, skip terminal commands and plan to trigger runs from the **Actions tab** in [Step 8](08-run-your-workflow.md#trigger-manually-via-github-actions-ui). `gh aw run` is not available without a terminal.
