...
Before you move to Step 7, confirm that Copilot CLI can reach the model from your Codespace. Catching an access problem here saves time — Step 7 requires Copilot access immediately, and diagnosing it mid-step is harder.
...
> [!IMPORTANT]
> If you see an error instead of a reply, fix the access issue before continuing. Check [github.com/settings/copilot](https://github.com/settings/copilot) to confirm Copilot is enabled on your account, then see [Side Quest: Configure GitHub Copilot for Agentic Workflows](side-quest-06-03-copilot-token.md).

- [ ] I ran the test prompt in Copilot CLI and received a reply (no error)
...
- [ ] Copilot CLI test prompt returned a reply (no error)
