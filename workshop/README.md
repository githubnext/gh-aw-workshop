...
- [How `dispatch-workflow` Orchestration Works](side-quest-28-01-dispatch-workflow-concepts.md) — explains why orchestration matters, what the `workflows` allowlist and `max: 1` fields protect against, and how to build a signal-to-action decision table before writing an orchestrator brief; branches from [Step 28](28-orchestrate-workflows.md).
