...
- `temporary_id`: `aw_report` (this is the parent issue handle used by child issues in step 8)
...
- sets `parent: "aw_report"` (preferred) to link directly to the parent from step 7
- use a numeric parent issue number only as fallback if the temporary id is unavailable
