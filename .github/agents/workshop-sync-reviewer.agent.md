...
For each check category, read **one** reference file at a time — only the file listed in the table above, and only when you are actively evaluating that category. Do **not** load all reference files at once — each file is large and loading them all would exceed the token budget for this subagent invocation.
